...
export default function PharmacistDashboard({ prescriptions = [], products = [], orders = [], statusChanges = [] }: any) {
  ...
  const recentActivities = [...prescriptions]
    ...
    .map((p: any) => ({
      ...
      detail: `${new Date(p.updated_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })} ${p.catatan_apoteker ? `┬À ${p.catatan_apoteker}` : ''}`,
                                      ...
                                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                          <div className="flex flex-col sm:flex-row sm:items-center flex-1 gap-4 md:gap-6">
                                              ...
                                              <div className="flex-1 min-w-[150px]">
                                                  ...
                                                  <p className="font-['Inter',sans-serif] text-[13px] text-[#6e7a70] truncate">
                                                      {rx.created_at ? new Date(rx.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : (rx.timeLabel || rx.date?.split(' ')[1])} ÔÇó {rx.created_at ? new Date(rx.created_at).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'}) : ''}
                                                  </p>
                                              </div>
                                                ...
                                                <div className="pb-1">
                                                    <p className="font-['Inter',sans-serif] text-[12px] font-semibold text-[#171d19]">
                                                        {history.status_sebelum
                                                            ? `${statusLabels[history.status_sebelum] || history.status_sebelum} ÔåÆ ${statusLabels[history.status_sesudah] || history.status_sesudah}`
